...
function initConditionsOrdering(update_php) {
	jQuery('#conditionsTable').tableDnD({
		onDragClass: 'rsform_dragged',
		onDragStop : function () {
			if (typeof update_php === 'undefined') {
				update_php = false;
			...
			var orders = document.getElementsByName('conditionorder[]');
			var cids = document.getElementsByName('conditionid[]');
			...
			if (update_php) {
				...
				var url = 'index.php?option=com_rsform&task=conditions.saveOrdering';
...
function changeFormAutoGenerateLayout(value) {
	...
	jQuery.post(url, data, function (response, status, jqXHR) {
		...
		setCodeMirrorReadonly('formLayout', value === '1');

		stateDone();
	}, 'json');
}

function setCodeMirrorReadonly(editorId, value)
{
	if (typeof Joomla.editors.instances[editorId] !== 'undefined') {
		var editor = Joomla.editors.instances[editorId];
		if (typeof editor.setOption === 'function') {
			editor.setOption('readOnly', value);
		} else if (typeof editor.disable === 'function') {
			editor.disable(!value);
		}
	}
}
...
function getValidationExtraText(value)
{
	...
	switch (value)
	{
		case 'restrictedwords':
			theText = Joomla.JText._('RSFP_COMP_FIELD_VALIDATIONEXTRARESTRICTEDWORDS');
			break;
		case 'regex':
...
function changeValidation(elem) {
	...
	if (elem.id === 'VALIDATIONRULE') {
		...
		if (extraId) {
			...
			if (['custom', 'numeric', 'alphanumeric', 'alpha', 'regex', 'sameas', 'minlength', 'maxlength', 'restrictedwords'].indexOf(elem.value) > -1) {
		...
		if (elem.value === 'multiplerules') {
			...
			try {
				...
				hasOldConfig = typeof savedExtraObject == 'object';
			} catch(e) {
		...
		for (i = 0; i < elem.length; i++) {
			if (elem[i].selected && ['custom', 'numeric', 'alphanumeric', 'alpha', 'regex', 'sameas', 'minlength', 'maxlength', 'restrictedwords'].indexOf(elem[i].value) > -1) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (fieldValue.length > 0) {
            ...
            for (var v = 0; v < values.length; v++) {
				...
				// Skip checks if we're using {if}
				if (value.indexOf('{if') > -1 || value.indexOf('{/if}') > -1) {
					wrongPlaceholder = notAnEmail = wrongDelimiter = false;
				}
