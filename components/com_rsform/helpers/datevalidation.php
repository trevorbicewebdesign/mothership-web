...
class RSFormProDateValidations
{
	...
	public static function fromtoday($day, $month, $year, $data = array())
	{
		try
		{
			...
			$selected = Factory::getDate($year.'-'.self::padding($month).'-'.self::padding($day).' 23:59:59');
	...
	public static function beforetodayexcluding($day, $month, $year, $data = array())
	{
		try
		{
			$today    = Factory::getDate();
			$today->setTime(0, 0, 0);
			$selected = Factory::getDate($year.'-'.self::padding($month).'-'.self::padding($day).' 00:00:00');
