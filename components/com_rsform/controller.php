...
class RsformController extends BaseController
{
	public function captcha()
	{
		...
		if (Factory::getDocument()->getType() != 'image')
		{
	...
	{
		Factory::getApplication()->triggerEvent('onRsformFrontendSwitchTasks');
	}
	...
	public function submissionsViewFile()
    {
		...
		if (strlen($hash) != 32)
		{
		    throw new Exception(Text::_('RSFP_VIEW_FILE_NOT_FOUND'));
		}
		...
		if ($result = $db->loadObject())
		{
			...
			{
                throw new Exception(Text::_('RSFP_VIEW_FILE_NOT_UPLOAD'));
			}
	...
	public function confirm()
    {
		...
		try
		{
			...
			{
				throw new Exception(Text::_('RSFP_SUBMISSION_CONFIRMED_ERROR'));
			}

			$query = $db->getQuery(true)
				->select($db->qn('SubmissionId'))
				->select($db->qn('FormId'))
				->select($db->qn('confirmed'))
			...
			{
				throw new Exception(Text::_('RSFP_SUBMISSION_ALREADY_CONFIRMED'));
			}
			...
			$app->enqueueMessage(Text::_('RSFP_SUBMISSION_CONFIRMED'), 'notice');
    ...
    public function deleteSubmission()
    {
		...
		{
			$this->checkToken();
			$this->setRedirect(Route::_('index.php?option=com_rsform&view=deletesubmission&hash=' . $hash, false));
        ...
        if (strlen($hash) === 32)
        {
            ...
            if ($submission = $db->loadObject())
            {
                ...
                $app->enqueueMessage(Text::_('COM_RSFORM_SUBMISSION_DELETED'));

				$this->setRedirect(Route::_('index.php?option=com_rsform&view=deletesubmission&layout=complete', false));
            }
            else
            {
                $app->enqueueMessage(Text::_($string), 'warning');
            }
        }
        else
        {
