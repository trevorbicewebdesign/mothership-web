...
<fieldset>
	<legend class="rsfp-legend"><?php echo Text::_('RSFP_' . $layoutGroup); ?></legend>
	<?php foreach ($layouts as $layout) { ?>
		<div class="rsform_layout_box">
			<label for="formLayout<?php echo ucfirst($layout); ?>" class="radio">
				<input type="radio" id="formLayout<?php echo ucfirst($layout); ?>" name="FormLayoutName" value="<?php echo $layout; ?>" onclick="saveLayoutName(this.value);" <?php if ($this->form->FormLayoutName == $layout) { ?>checked="checked"<?php } ?> /> <?php echo Text::_('RSFP_LAYOUT_'.str_replace('-', '_', $layout));?><br/>
			</label>
			<?php echo HTMLHelper::_('image', 'com_rsform/admin/layouts/' . $layout . '.gif', Text::_('RSFP_LAYOUT_'.str_replace('-', '_', $layout)), array('width' => 175), true); ?>
		</div>
	<?php } ?>
</fieldset>
<?php } ?>
...
<fieldset class="form-horizontal">
	<legend class="rsfp-legend"><?php echo Text::_('RSFP_FORM_HTML_LAYOUT_OPTIONS'); ?></legend>
	<?php echo $this->jform->renderFieldset('layout_options'); ?>
</fieldset>

<fieldset>
	<legend class="rsfp-legend"><?php echo Text::_('RSFP_FORM_HTML_LAYOUT'); ?></legend>
	<p>
		<button class="btn btn-warning" type="button" onclick="generateLayout(true);"><?php echo Text::_('RSFP_GENERATE_LAYOUT'); ?></button>
	</p>
