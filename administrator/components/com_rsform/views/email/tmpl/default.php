...
<p>
	<button class="btn btn-success" type="button" onclick="Joomla.submitbutton('emails.apply');"><?php echo Text::_('JAPPLY'); ?></button>
	<button class="btn btn-success" type="button" onclick="Joomla.submitbutton('emails.save');"><?php echo Text::_('JSAVE'); ?></button>
	<button class="btn btn-secondary" type="button" onclick="window.close();"><?php echo Text::_('JCANCEL'); ?></button>
</p>

<fieldset class="form-horizontal">
	<legend class="rsfp-legend"><?php echo Text::_('RSFP_FORM_EMAILS_NEW'); ?></legend>
	<?php
	if (!RSFormProHelper::getConfig('global.disable_multilanguage'))
	{
		...
		{
			?>
			<p><?php echo Text::sprintf('RSFP_YOU_ARE_EDITING_IN', $this->lang, $this->translateIcon); ?></p>
