...
Text::script('RSFP_BACKUP_SELECT');
Text::script('RSFP_ERROR');
Text::script('RSFP_STATUS');
Text::script('RSFP_STATUS_BACKING_UP_FORM_STRUCTURE_LEFT');
Text::script('RSFP_STATUS_BACKING_UP_FORM_SUBMISSIONS_LEFT');
Text::script('RSFP_STATUS_FINISHING_UP_SUBMISSIONS_FOR_FORM');
Text::script('RSFP_STATUS_COMPRESSING_FILES');
Text::script('RSFP_JSON_DECODING_ERROR');

HTMLHelper::_('script', 'com_rsform/admin/backup.js', array('relative' => true, 'version' => 'auto'));
	...
	<div id="formsList">
		<table class="table table-striped">
			...
			<tr>
				...
				<th class="title"><?php echo Text::_('RSFP_FORM_TITLE'); ?></th>
				<th class="title"><?php echo Text::_('RSFP_FORM_NAME'); ?></th>
				<th class="title" nowrap="nowrap" width="1%"><?php echo Text::_('RSFP_FORM_ID'); ?></th>
				<th class="title" nowrap="nowrap" width="1%"><?php echo Text::_('RSFP_SUBMISSIONS'); ?></th>
			</tr>
			</thead>
			<?php
			foreach ($this->forms as $i => $row)
			{
				...
				?>
				<tr>
					<td><?php echo HTMLHelper::_('grid.id', $i, $row->FormId); ?></td>
					<td><label for="cb<?php echo $i; ?>"><?php echo !empty($row->FormTitle) ? strip_tags($row->FormTitle) : '<em>' . Text::_('RSFP_FORM_DEFAULT_TITLE') . '</em>'; ?></label></td>
