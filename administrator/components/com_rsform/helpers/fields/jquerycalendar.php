...
class RSFormProFieldJqueryCalendar extends RSFormProField
{
	...
	public function getPreviewInput()
	{
		...
		return $codeIcon . ' ' . Text::_('RSFP_COMP_FVALUE_' . $layout);
	}
	...
	public function getFormInput() {
		...
		$hiddenValue = '';
        $hidden = Factory::getApplication()->input->get('hidden', array(), 'array');
		...
		if (!empty($hidden[$hiddenName])) {
			...
			if (!empty($value)) {
				...
				{
					$value = Factory::getDate('now')->format($format);
				}
		...
		$config = array(
			...
			'disabledDates'		 => $this->isCode($this->getProperty('DISABLEDDATES', '')),
			'disabledWeekDays'	 => $this->isCode($this->getProperty('DISABLEDWEEKDAYS', '')),
