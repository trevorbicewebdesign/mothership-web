...
class RSFormProFieldCaptcha extends RSFormProField
{
	...
	public function getPreviewInput()
	{
		...
		if ($type == 'INVISIBLE') {
			...
			// Image source
			$src = Route::_('index.php?option=com_rsform&task=captcha&componentId='.$componentId.'&format=image&sid='.mt_rand());
			// Image HTML
			$image = '<img'.
					 ' src="'.$src.'"'.
			...
			if ($refresh) {
				...
				$base 		= Route::_('index.php?option=com_rsform&task=captcha&componentId='.$componentId.'&format=image');
	...
	public function processValidation($validationType = 'form', $submissionId = 0)
	{
		...
		if ($this->getProperty('IMAGETYPE') == 'INVISIBLE')
		{
			...
			if (Factory::getApplication()->input->post->get($captchaCode, '', 'raw'))
			{
				return false;
			}
			...
			{
				if (Factory::getApplication()->input->post->get($word, '', 'raw'))
				{
					return false;
				}
			}
