    ...
    public static function buildJS($formId, $conditions)
    {
        ...
        if ($conditions)
        {
            ...
            foreach ($conditions as $condition)
            {
                if (!empty($condition->details))
                {
                    ...
                    foreach ($data->details as $detail)
                    {
                        ...
                        {
                            $scriptConditions .= sprintf('document.addEventListener(\'DOMContentLoaded\', function() { RSFormPro.Conditions.add(%1$d, %2$s, %3$s); });', $formId, json_encode($detail->ComponentName), $function);
			...
			if ($functions)
			{
				...
				$script .= sprintf('document.addEventListener(\'DOMContentLoaded\', function(){ RSFormPro.Conditions.addReset(%d); });', $formId);
			}
        }
