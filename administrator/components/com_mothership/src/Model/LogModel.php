...
class LogModel extends AdminModel
{
     ...
     * @since  3.2
     */
    public $typeAlias = 'com_mothership.log';
    ...
    protected function canDelete($record)
    {
        ...
        if (!$this->canDeleteLog($record->id)) {
            return false;
        }
    ...
    public function canDeleteLog(int $logId): bool
    {
        $db = Factory::getDbo();
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_mothership.log', 'log', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_mothership.edit.log.data', []);

        if (empty($data)) {
            $data = $this->getItem();
        }

        $this->preprocessData('com_mothership.log', $data);
