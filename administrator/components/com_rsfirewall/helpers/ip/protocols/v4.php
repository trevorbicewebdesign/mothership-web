...
class RSFirewallIPv4 extends RSFirewallIPBase implements RSFirewallIPInterface
{
	...
	public function toUnpacked() {
		...
		if (!isset($unpacked[1])) {
			throw new Exception(Text::sprintf('COM_RSFIREWALL_COULD_NOT_UNPACK_IP', $this->ip));
		}
	...
	public function toLong() {
		...
		if ($long === false) {
			throw new Exception(Text::sprintf('COM_RSFIREWALL_COULD_NOT_CONVERT_TO_LONG', $this->ip));
		}
	...
	public function cleanMask($mask) {
		...
		$mask = (int) $mask;
		if ($mask > 32 || $mask < 1) {
			throw new Exception(Text::sprintf('COM_RSFIREWALL_NETWORK_MASK_OUTSIDE_RANGE', $mask, '1-32'));
		}
