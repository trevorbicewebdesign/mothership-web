...
(function (factory) {
            ...
            "<div class='sp-container sp-hidden'>",
                "<div class='sp-palette-container'>",
                    "<div class='sp-palette sp-thumb sp-cf'></div>",
                      "<div class='sp-custom-more'>",
                        "<button type='button' class='sp-palette-toggle-cus'>",
    ...
    function spectrum(element, o) {
        ...
        var doc = element.ownerDocument,
            ...
            toggleButtonAddMore = container.find(".sp-palette-toggle-cus"),
        ...
        function initialize() {
            ...
            toggleButtonAddMore.on("click.spectrum", function (e) {
                e.stopPropagation();
                e.preventDefault();
        ...
        function convertSelectionPalette(selection){
            var data = selection.reverse();
            var unique = [],allSelect = [];
            if (opts.showPalette) {
                for (var i = 1; i <= data.length; i++) {
                    unique.push(data[i]);
                    if((i && i%8==0) || (i == data.length)){
                        allSelect.push(unique);
                        unique = [];
                    }
                }
        ...
        function drawPalette() {
            ...
            var html = [];
            html.push('<label class="sp-el-brand-color" ><span>Brand Color</span></label>');
            $.map(paletteArray, function (palette, i) {
                html.push(paletteTemplate(palette, currentColor, "sp-palette-row sp-palette-row-" + i, opts));
            });

            updateSelectionPaletteFromStorage();
            if (selectionPalette) {
                var selectColor = getUniqueSelectionPalette();
                var selectColorRev = convertSelectionPalette(selectColor);

                html.push('<label class="sp-el-custom" ><span>Custom</span></label>');
                // html.push(paletteTemplate(selectColorRev, currentColor, "sp-palette-row sp-palette-row-selection", opts));
                $.map(selectColorRev, function (palette, i) {
                    html.push(paletteTemplate(palette, currentColor, "sp-palette-row sp-palette-row-selection sp-palette-row-" + i, opts));
                });
            }
